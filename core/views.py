from django.shortcuts import render, redirect # <--- redirect es necesario para upload_product_image
from django.conf import settings # <--- Necesario para MEDIA_ROOT en upload_product_image
from django.core.files.storage import FileSystemStorage # <--- Para guardar archivos
# from .forms import ProductImageUploadForm # <--- Descomenta y asegúrate que este archivo exista si usas el formulario
import requests
import logging
import os # <--- Necesario para os.path.join
import time # <--- Para nombres de archivo únicos en upload_product_image
    ...
    try:
        ...
        for prod_data in api_productos_raw:
            ...
            if categoria_id in categories_with_products:
                # MODIFICADO: Añadir imagen_url al diccionario del producto
                categories_with_products[categoria_id]['products'].append({
                    ...
                    'imagen_url': prod_data.get('imagen_url') # <--- AÑADIDO: Pasar la imagen_url de la API
                })
            else:
